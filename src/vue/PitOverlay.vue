    ...
    <div class="bg-slate-950/98 border-[3px] border-[#ff5722] rounded-[10px] p-7 min-w-[420px] max-w-[560px] text-center shadow-[0_12px_48px_rgba(0,0,0,0.7)]">
      ...
      <template v-if="store.pitData.isSeason">
        <template v-if="!store.pitData.isSeasonComplete">
          ...
          <div class="text-[#ccc] text-sm mb-3">
            Vehicle: <strong class="text-white">{{ store.vehicleList.find(v => v.key === store.selectedVehicle)?.name ?? 'Default Truck' }}</strong>
          </div>
      ...
      <template v-else>
        <div class="grid gap-3 text-left text-sm text-slate-300 mb-4">
          <div>
            <label class="block text-slate-400 text-xs uppercase tracking-[2px] mb-2">Track</label>
            <select
              class="w-full bg-slate-900 border border-slate-700 text-white rounded-xl px-3 py-3 text-left text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
              :value="store.selectedTrack"
              @change="store.setSelectedTrack($event.target.value)"
            >
          ...
          <div>
            Vehicle
            <strong class="block text-white text-lg mt-1">{{ store.vehicleList.find(v => v.key === store.selectedVehicle)?.name ?? store.pitData.vehicleName ?? 'Default Truck' }}</strong>
          </div>
        </div>
