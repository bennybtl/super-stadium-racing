<template>
  ...
  >
    ...
    <hr class="border-t border-slate-700 my-4" />
    <div class="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500 mb-2">Edge Transitions</div>

    <label class="flex items-center justify-between gap-2 mb-3 text-[12px]">
      <span>Enable Transition Mesh</span>
      <input
        type="checkbox"
        :checked="editor.bridgeMesh.transitionEnabled"
        @change="editor.setBridgeMeshTransitionEnabled($event.target.checked)"
      />
    </label>

    <div class="flex justify-between mb-1 text-[12px]">
      <span>Transition Depth</span>
      <span>{{ editor.bridgeMesh.transitionDepth.toFixed(1) }}</span>
    </div>
    <input
      type="range" min="0" max="30" step="0.5"
      :value="editor.bridgeMesh.transitionDepth"
      :disabled="!editor.bridgeMesh.transitionEnabled"
      @input="editor.setBridgeMeshTransitionDepth(+$event.target.value)"
      class="w-full accent-[var(--accent)] mb-3 cursor-pointer"
    />

    <div class="flex justify-between mb-1 text-[12px]">
      <span>Transition Y Offset</span>
      <span>{{ editor.bridgeMesh.transitionYOffset.toFixed(2) }}</span>
    </div>
    <input
      type="range" min="-3" max="3" step="0.05"
      :value="editor.bridgeMesh.transitionYOffset"
      :disabled="!editor.bridgeMesh.transitionEnabled"
      @input="editor.setBridgeMeshTransitionYOffset(+$event.target.value)"
      class="w-full accent-[var(--accent)] mb-3 cursor-pointer"
    />
