<template>
  <div v-if="store.postRaceData" class="fixed inset-0 bg-black/90 flex items-center justify-center z-[1100] pointer-events-auto">
    <div class="bg-slate-950/95 border-3 border-red-500 rounded-3xl p-10 min-w-[560px] max-w-[720px] shadow-[0_12px_48px_rgba(0,0,0,0.7)]">

      <h2 class="text-red-400 uppercase tracking-[0.28em] text-sm text-center mb-4">
      ...
      <h2 class="text-red-400 uppercase tracking-[0.28em] text-sm text-center mb-4">Championship Standings</h2>
      <div class="flex flex-col gap-2">
        <div
          ...
          :class="{ 'bg-red-500/10 text-white font-semibold': s.isPlayer }"
        >
          <span class="w-6 font-semibold text-red-400">{{ s.position }}.</span>
      ...
      <button class="mx-auto mt-6 block rounded-2xl bg-gradient-to-b from-red-500 to-red-700 px-10 py-3 text-lg font-bold uppercase tracking-[0.2em] text-white shadow-lg shadow-red-950/30 transition hover:from-red-400 hover:to-red-600" @click="store.goToPit()">
        Head to the Pit →
      </button>
    </div>
  </div>
